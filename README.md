...
- **База есть** → проверка Atom-ленты `https://feed.rutracker.cc/atom/f/1605.atom` (50 последних раздач): новые добавляются в начало базы, обновлённые перезаписываются. Затем дообогащаются первые 100 записей базы (восполняется `title_id` и т.п.).
